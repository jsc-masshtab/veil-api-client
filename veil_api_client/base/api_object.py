...
class VeilApiObject:
    ...
    def __init__(self, client,
                 ...
                 retry_opts: Optional[VeilRetryConfiguration] = None) -> None:  # noqa: A003
    ...
    async def _get(self, url: str, extra_params: Optional[dict] = None,
                   retry_opts: Optional[VeilRetryConfiguration] = None) -> 'ClientResponse':
        """Layer for calling a client GET method.

        Note:
            retry_opts will override self.__retry_opts
        """
        if not retry_opts:
    ...
    async def tags_list(self, paginator: VeilRestPaginator = None):
        ...
        tag = VeilTag(client=self._client,
                      retry_opts=self.__retry_opts)
        # TODO: veil return too many objects if entity_class is not none.
